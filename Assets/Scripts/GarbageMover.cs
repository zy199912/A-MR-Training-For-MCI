...
public class GarbageMover : MonoBehaviour
{
    ...
    void Start()
    {
        // 查找场景中的BluetoothManager
        bluetoothManager = FindObjectOfType<BluetoothManager>();
        
        // 如果找到了BluetoothManager，注册为其游戏控制器
        if (bluetoothManager != null)
        {
            bluetoothManager.gameController = this;
        }
        else
        {
            Debug.LogWarning("未找到BluetoothManager组件！请确保场景中有包含BluetoothManager脚本的GameObject。");
        }
    }
    ...
    public void OnKickDetected()
    {
        ...
        {
            Debug.Log("IMU传感器检测到踢腿动作，触发W键效果");
            StartMovingToTrashCan();
        }
    }
    ...
    public void OnStompDetected()
    {
        ...
        {
            Debug.Log("IMU传感器检测到跺脚动作，触发S键效果");
            Destroy(gameObject);
        }
    }
}